...
forrest_clean <- forrest %>%
                 ...
                 # there are two sentences where the speaker is not defined
...
We will start by splitting our data into a training and testing set.
Then we will create cross-validation resamples of the training data to evaluate our models.
```{r}
set.seed(123)
...
```
## Define a tidmodel recipe and workflow.
...
rf_tune_results %>%
  ...
  labs(title = "Random Forest",
       subtitle = "ROC-AUC Scores of random forest models with varying number of trees, split-nodes and terminal nodes",
       caption = "Data: Master Thesis") +
  theme_minimal() +
  ylab("ROC-AUC") +
  theme_ft_rc() +
...
Our results here indicate that we did not overfit during the tuning process.
